...
calculate(Binary) when is_binary(Binary) ->
  calculate(Binary, <<0:16>>).

calculate(<<>>, <<CRC:16>>) -> CRC;
calculate(<<N:8, Dest/binary>>, <<CRC:16>>) ->
  calculate(Dest, <<((CRC bsl 8) bxor erlang:element(((CRC bsr 8) bxor N) band 16#00FF + 1, ?crc16tab)):16>>).
