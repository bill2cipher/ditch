...
%% @end
%%--------------------------------------------------------------------
-spec(start_link(Ref :: atom(), NbListener :: integer()) ->
  {ok, Pid :: pid()} | ignore | {error, Reason :: term()}).
start_link(Ref, NbListener) ->
  gen_server:start_link({local, Ref}, ?MODULE, [Ref, NbListener], []).
...
init([Ref, NbListener]) ->
  ...
  {ok, #state{max_conns = NbListener, ref = Ref}}.
...
handle_call({start_connection, Block, Args}, From, State) ->
  start_protocol_proc(Block, Args, From, State);
...
wake_up_waiters(Count, State) ->
  ...
  lists:foldl(fun({From, Args}, S) ->
    case start_protocol_proc2(S, Args) of
      {reply, R, S2} ->
...
start_protocol_proc(Block, Args, From, State) ->
  ...
  case Cur < Max of
    true -> start_protocol_proc2(State, Args);
...
start_protocol_proc2(State = #state{listener_sup = SupID, cur_conns = Cur}, Args) ->
