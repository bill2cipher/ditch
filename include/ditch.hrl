...
%%%
%%% @end
%%% Created : 07. 七月 2016 14:55
%%%-------------------------------------------------------------------
-author("jacky").

-define(PACKET_TIMEOUT, 2 * 120 * 1000).
-define(MAX_UDP_CONNS, 4096).
-define(FLUSH_INTERVAL, 20).
-define(UDP_SOCK_OPTS, [{port, 23456}, {active, 10}, {header, 0}, binary]).
...
-record(kcp_pcb, {
  ...
  incr = 0,
  snd_queue = ditch_queue:new(),
  rcv_queue = ditch_queue:new(),
  snd_buf = ditch_buffer:new(?KCP_WND_SND, undefined),
  rcv_buf = ditch_buffer:new(?KCP_WND_RCV, undefined),
  acklist = [],
  datalist = [],
  ackcount = 0,
  ackblock = 0,
